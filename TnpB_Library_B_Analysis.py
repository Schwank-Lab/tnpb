...
def importfiles(shortname, protolookup, uniquebarcodelookup):
    ...
    with gzip.open(path+filename, "rt") as fasta_file:
        ...
        for seq_record in SeqIO.parse(fasta_file, 'fastq'):
            barcode = str(seq_record.seq)[:7]  # barcode is only the last 6 bases of the 3trim sequence
...
tnpblibrary_templatedf = pd.read_csv('20230518_TnpB_TWIST2_nuclease_REFERENCE.csv')
tnpblibrary_templatedf['spacer_part'] = tnpblibrary_templatedf['oligofinal'].apply(lambda x: x[23:23+10])
tnpblibrary_templatedf['Spacer_Length'] = tnpblibrary_templatedf['spacer'].apply(lambda x: len(x) if not pd.isna(x) else 0)
...
tnpblibrary_templatedf['target_rv'] = tnpblibrary_templatedf.apply(lambda x: str(Seq(x.oligofinal).reverse_complement())[x.position_nick-8:x.position_nick+6],axis=1)
...
for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
    ...
    posnicknr = tnpblibrary_templatedf.columns.get_loc("position_nick")
    ...
    with gzip.open(exactpath + "Output/"+shortname+'_3trim.fastq.gz', 'rt') as fastqfile:
        ...
        for seq_record in SeqIO.parse(fastqfile, 'fastq'):
            ...
            nickposition = templatenumpy[variantindex,posnicknr]

            str(seq_record.seq.reverse_complement())[1:]
            target = str(seq_record.seq)[nickposition-25-8:nickposition-25+6] # Target sequence within the sequenced reads
            # print(target)
            # print(len(target))
    ...
    tnpblibrary_templatedf.to_csv('AnalysisFiles/20230518_'+shortname+'.csv')
