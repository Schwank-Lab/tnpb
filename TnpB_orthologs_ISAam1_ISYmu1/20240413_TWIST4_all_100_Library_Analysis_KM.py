...
editorlist = ['ISAam1', 'ISYmu1']
referencefilenamelist = ['20240405_TnpB_ISAam1_100member_REFERENCE.csv', '20240405_TnpB_ISYmu1_100member_REFERENCE.csv']
...
def importfiles(editor, shortname, protolookup, uniquebarcodelookup):
    '''
    ...
    '''
    diseasedict = {}
    filename = exactpath+'../fastq_twist4/'+editor+"/Output/"+shortname+'_5trim'+'.fastq.gz'
    ...
    filename = exactpath+'../fastq_twist4/'+editor+"/Output/"+shortname+'_3trim'+'.fastq.gz'
    with gzip.open(path+filename, "rt") as fasta_file:
        count = 0
        counttemp = 0
        print('Start barcode lookup loop...')
        for seq_record in SeqIO.parse(fasta_file, 'fastq'):
            barcode = str(seq_record.seq)[:7]  # barcode is only the last 7 bases of the 3trim sequence
            # print(str(seq_record.seq)[:7])
...
for editor, referencefilename in zip(editorlist, referencefilenamelist):
    print(editor)
    tnpblibrary_templatedf = pd.read_csv(referencefilename)
    ...
    # target with 6bp after nick and 8 bp before nick
    tnpblibrary_templatedf['target_rv'] = tnpblibrary_templatedf['oligofinal'].apply(lambda x: str(Seq(x).reverse_complement())[62:76])
    ...
    filelist = list_files1('../fastq_twist4/'+editor+'/Output/')
    ...
    for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
        ...
        diseasedict = importfiles(editor, shortname,protolookup, uniquebarcodelookup)
        ...
        libtargetnr = tnpblibrary_templatedf.columns.get_loc("target_rv")
        namenr = tnpblibrary_templatedf.columns.get_loc("Identifier")
        
        with gzip.open(exactpath + "../fastq_twist4/"+editor+"/Output/"+shortname+'_3trim.fastq.gz', 'rt') as fastqfile:
            ...
            for seq_record in SeqIO.parse(fastqfile, 'fastq'):
                ...
                target = str(seq_record.seq)[37:51] # Target sequence within the sequenced reads
                # print(target)
                # print(len(target))
        ...
        tnpblibrary_templatedf.to_csv('../fastq_twist4/'+editor+'/AnalysisFiles/20240413_'+shortname+'.csv')
