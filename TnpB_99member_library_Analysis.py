...
tnpblibrary_templatedf = pd.read_csv('20230518_TnpB_100member_REFERENCE.csv')
...
tnpblibrary_templatedf['target_rv'] = tnpblibrary_templatedf['oligofinal'].apply(lambda x: str(Seq(x).reverse_complement())[56:70])
...
for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
    ...
    with gzip.open(exactpath + "Output/"+shortname+'_3trim.fastq.gz', 'rt') as fastqfile:
        ...
        for seq_record in SeqIO.parse(fastqfile, 'fastq'):
            ...
            target = str(seq_record.seq)[36:50] # Target sequence within the sequenced reads
            # print(target)
            # print(len(target))
